...
class WxRequest {
	...
	__initMethods() {
		...
		// 遍历对象构造方法
		for (const key in instanceSource) {
			instanceSource[key].forEach((method, index) => {
				this[method.toLowerCase() + suffix] = (url, config) => this.__defaultRequest({ ...config, method, url });
			});
		}
	...
	__defaultRequest(config) {
		...
		if (typeof config === 'string') {
			config = { url: arguments[1], ...arguments[2] };
		}
