import { USER_API, APP_ID } from '../config/index';
...
function requestP(options = {}) {
	...
	// 统一注入约定的header
	const header = { Authorization: sessionId, ...options.header };
		...
		wx.login({
			success(r1) {
				if (r1.code) {
					// 获取sessionId
					requestP({
						url: `${USER_API}/wxlogin`,
						data: {
							app_id: APP_ID,
							code: r1.code,
							source_user_id:
								getApp() && getApp().globalData.source_user_id
									? getApp().globalData.source_user_id
									: null,
						},
						method: 'POST',
					})
